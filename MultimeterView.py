...
class MultimeterView (DeviceFrame):
    ...
    #This methods initiates all attributes in the class. It is usefull to prevent double usage     
        ...
        self.canva_setup = Canvas(self.labelFrame_setup, scrollregion=(0,0,0,500), bd=0, highlightthickness=0, bg=self.model.parameters_dict['backgroundColor'])
        ...
        self.intVar_radioValueCaliber = IntVar()
        ...
        self.entry_diode = Entry(self.frame_diode, textvariable=self.doubleVar_diode, state="readonly", width=20)
        self.entry_frequency = Entry(self.frame_frequency, textvariable=self.doubleVar_frequency, state="readonly", width=20)
        self.entry_period = Entry(self.frame_period, textvariable=self.doubleVar_period, state="readonly", width=20)
        ...
        self.continuityImg = self.continuityImg.resize((25, 22), Image.ANTIALIAS)
        ...
        self.radio_caliberACmA = Radiobutton(self.frame_ACI_caliber, text='mA', variable=self.intVar_radioValueCaliber, value=0, command=self.radio_caliber_callback)
        self.radio_caliberDCmA = Radiobutton(self.frame_DCI_caliber, text='mA', variable=self.intVar_radioValueCaliber, value=0, command=self.radio_caliber_callback)
        self.radio_caliberAC10A = Radiobutton(self.frame_ACI_caliber, text='10A', variable=self.intVar_radioValueCaliber, value=1, command=self.radio_caliber_callback)
        self.radio_caliberDC10A = Radiobutton(self.frame_DCI_caliber, text='10A', variable=self.intVar_radioValueCaliber, value=1, command=self.radio_caliber_callback)
        ...
        for item in self.model.devices_dict:
            if item in self.controller.instrument.address:
                ...
                if self.model.devices_dict[item][0] == "8845A":   
                    ...
                    self.img = self.img.resize((225, 115), Image.ANTIALIAS)
    ...
    #This method instanciates all the frames used as lines
        ...
        self.canva_setup.config(yscrollcommand= self.defilY_setup.set, height=280, width=220)
    ...
    #This methods activates or desactivates the modulation function
        ...
        for child in self.frame_period.winfo_children():
            ...
            self.radio_DCV.configure(fg="grey")
            self.radio_ACV.configure(fg="grey")
            self.radio_DCI.configure(fg="grey")
            self.radio_ACI.configure(fg="grey")
            self.radio_2WR.configure(fg="grey")
            self.radio_4WR.configure(fg="grey")
            self.radio_diode.configure(fg="grey")
            self.radio_continuity.configure(fg="grey")
            self.radio_frequency.configure(fg="grey")
            self.radio_period.configure(fg="grey")
    ...
    #This method is called when the amp caliber is changed 
        if self.intVar_radioValueCaliber.get() == 0:

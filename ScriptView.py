...
from tkinter import Button, Canvas, Entry, Frame, LabelFrame, Scrollbar, IntVar, StringVar
...
class ScriptView():
    ...
    #Constructor for the Sequence_view superclass
        ...
        self.root=root
        self.path = self.model.parameters_dict['outputFolder']
    ...
    #This method generates the Frame's parameters for the sequence
        self.dataFrame.configure(text="Script Editor", font=(12), padx=padx, pady=pady, bg=self.model.parameters_dict['backgroundColorConfiguration'])
        self.dataFrame.pack(fill="y", expand="no", side="left", anchor='nw')

        self.mainFrame.pack(fill="both", expand=True, side="left", anchor='nw', padx=5, pady=5)
        ...
        self.mainCanva.config(yscrollcommand= self.defilY_setup.set, xscrollcommand=self.defilX_setup.set, height=2000, width=2000)
        self.mainCanva.pack(fill="both", expand=True, side="left", anchor='nw')
    ...
    #This method adds a new command line to be displayed by ScriptView
        if (pos == None) and (command == None):
            ...
            sys.stdout("\nNew Command Line added at : " + str(number) + "\n")
            ...
            for item in self.listeCommand:
                item.line.pack(fill="x", expand=True, side="top", anchor='nw', pady=2)        
            self.renumberLine()        
            sys.stdout("\nNew Command Line added at : " + str(number) + "\n")
